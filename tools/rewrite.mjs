...
## Your job
Rewrite the article BODY below so it (a) addresses EVERY review note, (b) obeys every rule in the DNA, (c) keeps using the locked CSS components (.sous, .note--key/science/action/warning/quote, <mark>, .term, .ref, .fold--sources, data-label boxes), and (d) passes the Master Meta-Prompt's final-edit gates E1–E8 (claims audit with resolved DOIs, the ~⅓ cut, spine test, stack scan, placeholder scan, the Learning Contract — three verified facts a ten-year professional doesn't know — identity check, the 11 PM read). Do not invent new styling or a new template. ${hasMarkers ? "Output ONLY the new inner body HTML (what goes between BODY:BEGIN and BODY:END) — no <head>, no masthead, no footer." : "NOTE: this file has no BODY:BEGIN/END markers — add them around the body when you write it back, or apply by hand."}

The first law is **never lose focus**: every line earns the next. Open with the .sous block ONLY on Technique, Study Decoded and Equipment Review (the one line you would say out loud to your sous chef — the answer; everything below is the science behind it); elsewhere open on the template's own register. One idea per <h2>, re-hook the eye every ~150 words. Do NOT close on a takeaway box: retired publication-wide 2026-09-05. No emoji. No banned words (delicious, yummy, mouthwatering, elevated, curated, artisanal, superfood, game-changer, hack). Name every source — no "studies show" without the study. British cooking spelling.
