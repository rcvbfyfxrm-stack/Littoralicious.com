...
## Rules
Obey every rule in the DNA below, and run the Master Meta-Prompt protocol below it — phase 1 HUNT before drafting (resolve DOIs first; find six Learning-Contract facts, keep three), phase 2 BUILD in the template's opening register, phase 3 FINAL EDIT as a separate adversarial pass (gates E1–E8, cut ~⅓). The Learning Contract is law: three verified facts a ten-year professional chef doesn't know, one in the first two screens. The first law is **never lose focus** — on Technique, Study Decoded and Equipment Review ONLY, open with the .sous block (the one line you would say out loud to your sous chef: the answer, imperative and specific; everything below is the science behind it); on every other template open on that template's own register and keep the same discipline in prose. Then the .article-toc sum-up, one idea per <h2>, re-hook every ~150 words. Do NOT close on a takeaway box — that is retired publication-wide; the sous-chef line already carries it. Cite with numbered <sup class="ref"> markers, never parentheticals, and fold the sources at the foot. Hold all three pillars (grandmother warmth · scientist precision, sourced · yacht-chef directness). NO emoji. NO banned words. Name every source — no "studies show" without the study. British cooking spelling. Use only the locked CSS components (.sous, .note--key/science/action/warning/quote, <mark>, .term, .ref, .fold--sources, data-label boxes). Output ONLY the inner body HTML (between BODY:BEGIN and BODY:END).
