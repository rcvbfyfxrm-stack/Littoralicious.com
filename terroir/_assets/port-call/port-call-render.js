    ...
    function boot() {
        ...
        function renderBerths() {
            ...
            target.innerHTML = berths.map((v, i) => {
                ...
                const nbhd = v.neighborhood ? ' <span class="pcv-berth__nbhd" style="font-family:\'JetBrains Mono\',monospace;font-size:10px;letter-spacing:1px;text-transform:uppercase;color:var(--accent);opacity:.85;">· ' + escapeHTML(v.neighborhood) + '</span>' : '';
                return '<div class="pcv-berth" id="venue-' + v.id + '" data-venue-id="' + v.id + '">' +
                    ...
                    '<div class="pcv-berth__name">' + escapeHTML(v.name) + nbhd + '</div>' +
        ...
        function renderCard(v, rank) {
            ...
            const products = productPills(v.productTags);
            const meta = [];
            if (v.neighborhood) meta.push('<div class="pcv-card__meta-row"><span>Neighborhood</span><span>' + escapeHTML(v.neighborhood) + '</span></div>');
            ...
            const nbhdInline = v.neighborhood ? ' <span class="pcv-card__nbhd" style="font-family:\'JetBrains Mono\',monospace;font-size:10px;letter-spacing:1px;text-transform:uppercase;color:var(--ink-mut);font-weight:400;">· ' + escapeHTML(v.neighborhood) + '</span>' : '';
            return '<article class="pcv-card" data-venue-id="' + v.id + '" data-cat="' + v.cat + '" id="venue-' + v.id + '">' +
                ...
                '<div class="pcv-card__body">' +
                    ...
                    '<div class="pcv-card__name">' + escapeHTML(v.name) + badge + offCity + nbhdInline + '</div>' +
