...
window.PANTRY_ENRICH = (function () {
  ...
  function enrichItem(item, cat, section, ctx) {
    ...
    for (const rule of portions) {
      ...
      if (qty != null) {
        // Round-up for discrete physical units (you can't order half a tin)
        const wholeUnits = new Set(["tin","tins","pcs","pc","piece","jar","jars","bunch","case","box","pack","birds","racks","legs","claws","side","sides","lobes","tray","trays","log","logs","sheets","wheel","stick","sticks","book","books","tube","tubes","bottle","cans"]);
        ...
        const mathLine = rule.perPaxOnce != null
          ? `${rule.perPaxOnce}${rule.unit || ""} × ${pax} ${paxLabel} → ${qty} ${rule.unit || ""}`
          : `${rule.perPaxPerDay}${rule.unit || ""} × ${pax} ${paxLabel} × ${days}d ÷ ${rule.divisor || 1} = ${qty} ${rule.unit || ""}`;
        result.portion = { qty, unit: rule.unit, note: rule.note, role, math: mathLine };
      }
      break;
    }
