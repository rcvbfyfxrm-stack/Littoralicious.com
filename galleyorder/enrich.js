...
window.PANTRY_ENRICH = (function () {
  ...
  // ---------------------------------------------------------------------------
  const portions = [
    // ====================== MAINSTREAM CORE — meat / fish / dairy / bakery / pantry ===
    // Only rules with mainstream:true are auto-filled by the "Fill by PAX" button.
    // Everything else (lamb, duck, game, sashimi, Wagyu, foie gras, caviar, modernist,
    // ice-cream, equipment, exotic produce) must be added MANUALLY by the chef.
    // Chicken > Beef > Pork > Lamb in frequency. Mainstream is conservative on purpose.

    // --- CHICKEN (high frequency, 2-3 services / week) ---
    { cat: "meat", role: "all", name: /chicken breast(?!.*supreme)|breast.*chicken|^chicken fillet/i,
      perPaxPerDay: 0.18, divisor: 3, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 180 g chicken breast × 2-3 services / week" },
    { cat: "meat", role: "all", name: /chicken tender|chicken goujon/i,
      perPaxPerDay: 0.15, divisor: 4, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 150 g chicken tenders / pax / week" },

    // --- BEEF (basic everyday cuts; luxury wagyu/cote stays manual) ---
    { cat: "meat", role: "all", name: /^(beef striploin|striploin.*standard|striploin angus|black angus striploin|black angus ribeye|ribeye.*standard|^prime rib roast)/i,
      perPaxOnce: 0.25, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 250 g basic steak / pax (1 service / week)" },
    { cat: "meat", role: "crew", name: /^minced beef|^ground beef|^beef ground|hamburger meat/i,
      perPaxPerDay: 0.15, divisor: 7, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 150 g ground beef / crew / service (bolognese, tacos, burger)" },

    // --- PORK (only basic cuts mainstream; Iberico/mangalitsa stays manual) ---
    { cat: "meat", role: "all", name: /\bbacon\b(?!.*lardons|.*flavor|.*bits)/i,
      perPaxPerDay: 0.05, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 50 g bacon / pax / day breakfast" },
    { cat: "meat", role: "all", name: /pork loin|pork tenderloin(?!.*iberico)|pork chop(?!.*mangalitsa|.*iberico)|pork cutlet|pork escalope/i,
      perPaxOnce: 0.18, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 180 g pork loin / pax (1 service / week)" },
    { cat: "meat", role: "all", name: /^chorizo|^salami(?!.*premium|.*tartufo|.*acorn)|^italian sausage|breakfast sausage|^toulouse sausage|merguez/i,
      perPaxOnce: 0.1, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 100 g sausage / pax (1 service / week)" },
    { cat: "meat", role: "all", name: /sliced ham(?!.*iberico)|cooked ham|honey ham|prosciutto cotto|^jambon blanc|deli ham/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 40 g sliced ham / pax / day sandwich + breakfast" },

    // --- LAMB / DUCK / VEAL / GAME — explicitly NOT mainstream ---
    // (Existing rules already lack mainstream flag → automatically skipped.)
    // Chef adds these manually when planning lamb/duck/game services.

    // --- FISH (basic mainstream) ---
    { cat: "fish", role: "all", name: /^atlantic salmon|^norwegian salmon|^scottish salmon|^salmon fillet(?!.*king|.*ora|.*wild)|^salmon side|^salmon loin(?!.*king)/i,
      perPaxPerDay: 0.18, divisor: 3, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 180 g Atlantic salmon × 2-3 services / week" },
    { cat: "fish", role: "all", name: /^sea bass|^branzino|^european bass|^loup de mer/i,
      perPaxOnce: 0.18, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 180 g sea bass fillet / pax (1 service)" },
    { cat: "fish", role: "all", name: /^cod(?!.*black|.*sable|.*salt)|^haddock|^pollock|^hake$/i,
      perPaxOnce: 0.18, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 180 g white fish / pax (1 service)" },
    { cat: "fish", role: "all", name: /^shrimp(?!.*spot|.*amaebi|.*head-on)|^peeled prawn|^prawn(?!.*tiger|.*spot|.*carabineros|.*head-on)|^white shrimp|^pink shrimp/i,
      perPaxOnce: 0.12, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 120 g peeled shrimp / pax (1 service)" },
    { cat: "fish", role: "all", name: /^mussel|moules|^clam(?!.*razor|.*geoduck)|^vongole/i,
      perPaxOnce: 0.4, unit: "kg", minQty: 2, mainstream: true,
      note: "MAINSTREAM · 400 g mussels-in-shell / pax (1 service)" },
    { cat: "fish", role: "all", name: /^tuna(?!.*sashimi|.*bluefin|.*chu-toro|.*toro$|.*akami)|^yellowfin steak|^tuna steak|^ahi steak/i,
      perPaxOnce: 0.15, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 150 g tuna steak / pax (1 service, seared not raw)" },
    { cat: "fish", role: "all", name: /^calamari|^squid(?!.*ink)|^baby squid/i,
      perPaxOnce: 0.1, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 100 g calamari / pax (1 service)" },

    // --- CHEESE & DAIRY (basics — premium aged stays manual) ---
    { cat: "cheese-dairy", role: "all", name: /^milk\s*—\s*(whole|skim|semi)/i,
      perPaxPerDay: 0.3, unit: "L", minQty: 6, mainstream: true,
      note: "MAINSTREAM · 300 mL milk / pax / day (coffee, cooking, breakfast)" },
    { cat: "cheese-dairy", role: "all", name: /^lurpak|^kerrygold/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 40 g butter / pax / day (cook + spread)" },
    { cat: "cheese-dairy", role: "all", name: /heavy cream|whipping cream|crème fleurette|crème épaisse|crème fraîche$|sour cream/i,
      perPaxPerDay: 0.05, unit: "L", minQty: 2, mainstream: true,
      note: "MAINSTREAM · 50 mL cream / pax / day (sauces + finishing)" },
    { cat: "cheese-dairy", role: "all", name: /^eggs|^large eggs|^free.range eggs|^organic eggs|^pastured eggs/i,
      perPaxPerDay: 1.5, unit: "pcs", minQty: 24, mainstream: true,
      note: "MAINSTREAM · 1.5 eggs / pax / day (breakfast + cook)" },
    { cat: "cheese-dairy", role: "all", name: /^yogurt\s*—\s*greek/i,
      perPaxPerDay: 0.1, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 100 g yogurt / pax / day (breakfast)" },
    { cat: "cheese-dairy", role: "all", name: /^cream cheese|^philadelphia|^st moret|^kiri/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 20 g cream cheese / pax / day (bagel + dip)" },
    { cat: "cheese-dairy", role: "all", name: /^mozzarella(?!.*buffalo)|^fior di latte|low-moisture mozzarella/i,
      perPaxOnce: 0.08, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 80 g mozzarella / pax (2 services / week)" },
    { cat: "cheese-dairy", role: "all", name: /^feta|^greek feta|^bulgarian feta/i,
      perPaxOnce: 0.05, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 50 g feta / pax (1-2 services)" },
    { cat: "cheese-dairy", role: "all", name: /^cheddar(?!.*aged 24|.*5 year|.*reserve)|^mild cheddar|^mature cheddar/i,
      perPaxOnce: 0.06, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 60 g cheddar / pax (sandwich + grilled cheese)" },
    { cat: "cheese-dairy", role: "all", name: /^parmigiano(?!.*24|.*36|.*reserve)|^grated parmesan|^parmesan(?!.*aged)|^grana padano/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 20 g parmesan / pax / day (pasta + risotto + plate)" },
    { cat: "cheese-dairy", role: "all", name: /^goat cheese|^chèvre|^fresh goat|crottin/i,
      perPaxOnce: 0.06, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 60 g goat cheese / pax (1-2 services)" },

    // --- BAKERY ---
    { cat: "bakery", role: "all", name: /^sourdough loaves?\b|^country loaf|^rustic loaf|^white loaf|^pain de campagne$|^miche$|^boule$/i,
      perPaxPerDay: 0.5, divisor: 3, unit: "pcs", minQty: 2, mainstream: true,
      note: "MAINSTREAM · 1 loaf per 6 pax-days" },
    { cat: "bakery", role: "all", name: /^sliced bread|^sandwich bread|^pain de mie|^toast bread|^white sliced|^whole-?wheat sliced/i,
      perPaxPerDay: 0.3, divisor: 5, unit: "pcs", minQty: 2, mainstream: true,
      note: "MAINSTREAM · 1 loaf per 10 pax-days" },
    { cat: "bakery", role: "all", name: /^croissant(?!.*amande|.*chocolat)/i,
      perPaxPerDay: 1.5, unit: "pcs", minQty: 12, mainstream: true,
      note: "MAINSTREAM · 1.5 croissants / pax / day breakfast" },
    { cat: "bakery", role: "all", name: /^pain au chocolat|^chocolate croissant|^chocolatine/i,
      perPaxPerDay: 0.8, unit: "pcs", minQty: 6, mainstream: true,
      note: "MAINSTREAM · ~1 pain au chocolat / pax / day breakfast" },
    { cat: "bakery", role: "all", name: /^brioche\s*\(|^brioche loaf|^brioche bun(?!.*premium)|^brioche tressée/i,
      perPaxOnce: 0.3, unit: "pcs", minQty: 4, mainstream: true,
      note: "MAINSTREAM · 1 brioche / 3 pax / charter" },
    { cat: "bakery", role: "all", name: /^bread rolls?|^dinner rolls?|^bun(?!.*hot dog|.*burger)|^pavé|^petit pain/i,
      perPaxPerDay: 1, unit: "pcs", minQty: 12, mainstream: true,
      note: "MAINSTREAM · 1 dinner roll / pax / day" },
    { cat: "bakery", role: "all", name: /^burger bun|^brioche bun|^hamburger bun/i,
      perPaxOnce: 1.5, unit: "pcs", minQty: 12, mainstream: true,
      note: "MAINSTREAM · 1.5 burger buns / pax / week" },
    { cat: "bakery", role: "all", name: /^baguette\s*—\s*par.baked|^baguette$|^baguette\s*\(/i,
      perPaxPerDay: 0.5, unit: "pcs", minQty: 4, mainstream: true,
      note: "MAINSTREAM · 1 baguette / 2 pax / day" },
    { cat: "bakery", role: "all", name: /^english muffin|^muffin anglais/i,
      perPaxOnce: 1.5, unit: "pcs", minQty: 12, mainstream: true,
      note: "MAINSTREAM · 1.5 English muffins / pax / week breakfast" },

    // --- GRAINS, PASTA, RICE ---
    { cat: "grains-pasta", role: "all", name: /^spaghetti(?:\s|\/|$)|^penne(?:\s|\/|$)|^rigatoni|^fusilli|^farfalle|^linguine|^tagliatelle|^pasta\s*—\s*(de cecco|rummo|garofalo|felicetti|barilla)/i,
      perPaxOnce: 0.08, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 80 g dry pasta / pax × 2 services / week" },
    { cat: "grains-pasta", role: "all", name: /indian basmati\s*—\s*12.month|^brown basmati$|^jasmine rice$|^thai jasmine|^long.grain white rice$/i,
      perPaxOnce: 0.08, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 80 g rice / pax × 2-3 services / week" },
    { cat: "grains-pasta", role: "all", name: /^arborio$|^vialone nano$|^carnaroli$|^carnaroli riserva|^bomba\s*—\s*calasparra|^koshihikari\s*—\s*uonuma/i,
      perPaxOnce: 0.07, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 70 g specialty rice / pax × 1-2 services / week" },
    { cat: "grains-pasta", role: "all", name: /^quinoa(?!.*black|.*red mix)|^white quinoa|^couscous(?!.*israeli|.*mograbieh)/i,
      perPaxOnce: 0.06, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 60 g quinoa/couscous / pax × 1-2 services" },
    { cat: "grains-pasta", role: "all", name: /^oat|^rolled oats|^porridge oats|^muesli|^granola(?!.*house)/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 40 g oats / pax / day breakfast" },
    { cat: "grains-pasta", role: "all", name: /^t55\s*—|^all.purpose\s*\/?\s*plain flour|^italian 00\s*—\s*caputo cuoco/i,
      perPaxOnce: 0.05, unit: "kg", minQty: 2, mainstream: true,
      note: "MAINSTREAM · 50 g flour / pax × charter (baking, dust, sauces)" },

    // --- WESTERN / PANTRY CORE ---
    { cat: "western", role: "all", name: /^evoo\s*—\s*(cooking|finishing)$/i,
      perPaxPerDay: 0.04, unit: "L", minQty: 2, mainstream: true,
      note: "MAINSTREAM · 40 mL olive oil / pax / day cook + dress" },
    { cat: "western", role: "all", name: /^maldon\s+sea\s+salt|^fine sea salt$|^kosher salt$/i,
      perPaxOnce: 0.05, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 1 box / charter" },
    { cat: "western", role: "all", name: /^black pepper|^whole black peppercorn|^ground black pepper|^poivre noir/i,
      perPaxOnce: 1, unit: "jar", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 1 jar / charter" },
    { cat: "western", role: "all", name: /^balsamic vinegar(?!.*aged|.*tradizionale)|^white wine vinegar|^red wine vinegar|^apple cider vinegar/i,
      perPaxOnce: 1, unit: "bottle", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 1 bottle / charter" },
    { cat: "western", role: "all", name: /^dijon mustard|^moutarde de dijon|^french mustard/i,
      perPaxOnce: 1, unit: "jar", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 1 jar / charter" },
    { cat: "western", role: "all", name: /^mayonnaise(?!.*kewpie|.*japanese|.*truffle|.*saffron)|^mayo(?!nnaise)|^hellmann's|^french mayo|^homemade mayo/i,
      perPaxOnce: 1, unit: "jar", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 1 jar / charter sandwiches + dressings" },
    { cat: "western", role: "all", name: /^ketchup|^tomato ketchup|^heinz ketchup/i,
      perPaxOnce: 1, unit: "bottle", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 1 bottle / charter" },
    { cat: "western", role: "all", name: /^canned tomato|^crushed tomato|^chopped tomato|^pelati|^san marzano canned|^whole peeled tomato/i,
      perPaxPerDay: 0.07, unit: "kg", minQty: 4, mainstream: true,
      note: "MAINSTREAM · 1 can / day · sauces + braises" },
    { cat: "western", role: "all", name: /^tomato paste|^concentrate tomate|^double concentrate/i,
      perPaxOnce: 1, unit: "tube", minQty: 2, mainstream: true,
      note: "MAINSTREAM · 1-2 tubes / charter" },
    { cat: "western", role: "all", name: /^white sugar|^caster sugar|^granulated sugar|^sucre blanc/i,
      perPaxOnce: 0.05, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 50 g sugar / pax / charter" },
    { cat: "western", role: "all", name: /^honey\s*—\s*(table|clover|wildflower|standard)|^miel\s*—\s*(toutes|fleur|trèfle)/i,
      perPaxOnce: 0.05, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · ~1 jar honey / charter" },
    { cat: "western", role: "all", name: /^chicken stock|^vegetable stock|^beef stock|^bouillon cube|^stock cube|^court bouillon/i,
      perPaxOnce: 1, unit: "L", minQty: 4, mainstream: true,
      note: "MAINSTREAM · 4 L stock / charter base" },
    { cat: "western", role: "all", name: /^canned chickpeas|^canned cannellini|^canned kidney beans|^chickpeas in water|^white beans in water/i,
      perPaxOnce: 0.3, unit: "tin", minQty: 4, mainstream: true,
      note: "MAINSTREAM · 4 tins / charter" },
    { cat: "western", role: "all", name: /^canned tuna|^tuna in oil|^tuna in water/i,
      perPaxOnce: 0.5, unit: "tin", minQty: 6, mainstream: true,
      note: "MAINSTREAM · ~6 tins tuna / charter lunch + niçoise" },
    { cat: "western", role: "all", name: /^worcestershire|^lea & perrins/i,
      perPaxOnce: 1, unit: "bottle", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 1 bottle / charter" },
    { cat: "western", role: "all", name: /^soy sauce|^shoyu(?!.*aged|.*usukuchi)|^kikkoman/i,
      perPaxOnce: 1, unit: "bottle", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 1 bottle / charter" },

    // --- Western butter (table butter sits in western "Other Oils & Fats") ---
    { cat: "western", role: "all", name: /^butter\s*—\s*unsalted$|^butter\s*—\s*salted$/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 1, mainstream: true,
      note: "MAINSTREAM · 40 g butter / pax / day (table + cook)" },
    { cat: "western", role: "all", name: /^cultured.*fermented butter$/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 20 g cultured butter / pax / day (finishing)" },

    // --- Cheese-dairy ricotta ---
    { cat: "cheese-dairy", role: "all", name: /^ricotta\s*—\s*cow|^ricotta$/i,
      perPaxOnce: 0.05, unit: "kg", minQty: 0.5, mainstream: true,
      note: "MAINSTREAM · 50 g ricotta / pax (1-2 services)" },

    // --- Bakery: eggs (catalog has "Eggs · Egg Products" section) ---
    { cat: "bakery", role: "all", name: /^eggs?\s*—\s*(large|whole|medium|free.range|organic|pastured|standard)|^eggs?$|^large eggs|^whole eggs/i,
      perPaxPerDay: 1.5, unit: "pcs", minQty: 24, mainstream: true,
      note: "MAINSTREAM · 1.5 eggs / pax / day breakfast + cook" },
    ...
    { cat: "fish", role: "crew",  name: /atlantic salmon/i,
      perPaxPerDay: 0.15, unit: "kg", mainstream: true, note: "150 g crew portion (1 of 7 services)", divisor: 7 },

    // WHITE FILLETS — all-PAX (mixed service guest + crew over week)
    { cat: "fish", role: "all", section: /white fillet/i,
      perPaxPerDay: 0.16, unit: "kg", mainstream: true, note: "160 g portion / pax (mix guest + crew)", divisor: 7 },
    { cat: "fish", role: "all", name: /^mahi mahi|^cod \(atlantic\)|^hake$|^flounder|monkfish/i,
      perPaxPerDay: 0.15, unit: "kg", mainstream: true, note: "150 g portion / pax", divisor: 7 },
    ...
    { cat: "fish", role: "guest", name: /^tiger prawn/i,
      perPaxOnce: 3, unit: "pcs", mainstream: true, note: "3 head-on U-6 / guest" },
    ...
    { cat: "meat", role: "guest", name: /whole beef tenderloin/i,
      perPaxOnce: 0.180, unit: "kg", mainstream: true, note: "180 g portion from whole loin / guest" },
    ...
    { cat: "meat", role: "crew",  name: /beef mince — chuck/i,
      perPaxPerDay: 0.150, unit: "kg", mainstream: true, note: "150 g crew bolognese / crew / service", divisor: 7 },
    { cat: "meat", role: "guest", name: /beef mince — premium|sirloin grind/i,
      perPaxOnce: 0.150, unit: "kg", mainstream: true, note: "150 g sirloin grind / guest" },
    ...
    { cat: "meat", role: "crew",  name: /grass-fed beef burger patties|crew burger/i,
      perPaxOnce: 1, unit: "pcs", mainstream: true, note: "1 patty / crew" },
    { cat: "meat", role: "guest", name: /slider patties/i,
      perPaxOnce: 2, unit: "pcs", mainstream: true, note: "2 sliders / guest (canapé)" },

    // BRAISES — all-PAX (mixed service)
    { cat: "meat", role: "all", name: /^short ribs — bone-in/i,
      perPaxOnce: 1, unit: "pcs", mainstream: true, note: "1 rib / pax" },
    { cat: "meat", role: "crew", name: /boneless short rib/i,
      perPaxPerDay: 0.200, unit: "kg", mainstream: true, note: "200 g crew braise / crew", divisor: 7 },
    { cat: "meat", role: "guest", name: /beef cheeks(?! \(smoked\))/i,
      perPaxOnce: 1, unit: "pcs", mainstream: true, note: "1 cheek / guest" },
    { cat: "meat", role: "all", name: /^oxtail/i,
      perPaxOnce: 0.200, unit: "kg", note: "200 g / pax oxtail" },
    { cat: "meat", role: "all", name: /^brisket/i,
      perPaxOnce: 0.250, unit: "kg", mainstream: true, note: "250 g / pax sliced brisket" },
    ...
    { cat: "meat", role: "all", name: /pork belly.*skin-on/i,
      perPaxOnce: 0.180, unit: "kg", mainstream: true, note: "180 g pork belly / pax" },
    { cat: "meat", role: "crew", name: /^pork mince/i,
      perPaxPerDay: 0.150, unit: "kg", mainstream: true, note: "150 g crew / crew", divisor: 7 },
    ...
    { cat: "meat", role: "all", name: /label rouge.*chicken|organic chicken|whole.*chicken/i,
      perPaxOnce: 0.25, unit: "birds", mainstream: true, note: "1 bird / 4 pax" },
    { cat: "meat", role: "all", name: /chicken supreme/i,
      perPaxOnce: 1, unit: "pcs", mainstream: true, note: "1 supreme / pax" },
    { cat: "meat", role: "all", name: /chicken thigh.*boneless/i,
      perPaxOnce: 1, unit: "pcs", mainstream: true, note: "1 thigh / pax" },
    { cat: "meat", role: "crew", name: /chicken drumsticks/i,
      perPaxPerDay: 0.300, unit: "kg", mainstream: true, note: "300 g drumsticks / crew / service", divisor: 7 },
    { cat: "meat", role: "crew", name: /chicken wings/i,
      perPaxPerDay: 0.250, unit: "kg", mainstream: true, note: "250 g wings / crew", divisor: 7 },
    ...
    // ====================== CAVIAR & LUXURY ROE =============================
    { cat: "caviar", role: "guest", name: /^caviar.*beluga.*spécial réserve|caviar.*beluga.*tsar/i,
      perPaxOnce: 0.5, unit: "tin", minQty: 1, note: "1 × 30 g tin per 2 guests · ultra-luxury service" },
    { cat: "caviar", role: "guest", name: /^caviar.*beluga.*royal|^caviar.*ossetra.*tsar|^caviar.*ossetra.*spécial/i,
      perPaxOnce: 0.5, unit: "tin", minQty: 1, note: "1 × 30 g tin per 2 guests" },
    { cat: "caviar", role: "guest", name: /^caviar.*ossetra.*royal|^caviar.*sevruga|^caviar.*alverta|^caviar.*baïka|^caviar.*daurenki|^caviar.*kaluga|^caviar.*oscietre/i,
      perPaxOnce: 0.4, unit: "tin", minQty: 1, note: "1 × 30 g tin per ~2.5 guests" },
    { cat: "caviar", role: "guest", name: /^caviar.*baerii|^caviar.*pressé/i,
      perPaxOnce: 0.3, unit: "tin", minQty: 1, note: "1 × 30 g tin per 3 guests (everyday luxury)" },
    { cat: "caviar", role: "all", name: /salmon roe|ikura/i,
      perPaxOnce: 15, unit: "g", minQty: 100, note: "15 g ikura / pax" },
    { cat: "caviar", role: "all", name: /trout roe/i,
      perPaxOnce: 10, unit: "g", minQty: 100, note: "10 g trout roe / pax" },
    { cat: "caviar", role: "guest", name: /bottarga/i,
      perPaxOnce: 0.1, unit: "pcs", minQty: 1, note: "1 lobe per ~10 guests · grated service" },
    { cat: "caviar", role: "all", name: /tobiko|masago/i,
      perPaxOnce: 8, unit: "g", minQty: 250, note: "8 g flying-fish roe / pax" },
    { cat: "caviar", role: "guest", name: /sea urchin|^uni/i,
      perPaxOnce: 0.4, unit: "tray", minQty: 1, note: "1 tray per ~2.5 guests" },

    // ====================== SMOKED FISH =====================================
    { cat: "caviar", role: "all", name: /whole smoked.*salmon|whole wild smoked|whole smoked scottish/i,
      perPaxPerDay: 0.04, unit: "side", minQty: 1, divisor: 5, note: "1 side feeds ~25 pax over 5 services" },
    { cat: "caviar", role: "all", name: /pre-sliced.*smoked.*salmon|pre.*sliced.*smoked.*scottish|pre.*sliced.*smoked.*baltic/i,
      perPaxPerDay: 0.05, unit: "tray", minQty: 1, divisor: 5, note: "1 tray feeds ~20 pax over 5 services" },
    { cat: "caviar", role: "crew", name: /smoked salmon.*small pack/i,
      perPaxPerDay: 0.2, unit: "pack", minQty: 1, divisor: 7, note: "Crew breakfast / sandwich pack" },
    { cat: "caviar", role: "guest", name: /smoked eel|smoked sturgeon|smoked halibut|smoked trout/i,
      perPaxOnce: 0.25, unit: "pack", minQty: 1, note: "1 × 200 g pack per 4 guests" },
    { cat: "caviar", role: "all", name: /gravlax/i,
      perPaxPerDay: 0.04, unit: "side", minQty: 1, divisor: 5, note: "1 side feeds ~25 pax over 5 services" },

    // ====================== CAVIAR ACCOMPANIMENTS ===========================
    { cat: "caviar", role: "guest", name: /^blini/i,
      perPaxOnce: 6, unit: "pcs", minQty: 30, note: "6 blinis / guest with caviar service" },
    { cat: "caviar", role: "guest", name: /crème fraîche|smetana/i,
      perPaxOnce: 25, unit: "g", minQty: 200, note: "25 g crème / pax" },

    // ====================== ICE CREAM & SORBETS =============================
    { cat: "ice-cream", role: "all", section: /premium ice cream|sorbets/i,
      perPaxPerDay: 80, unit: "g", minQty: 2500, divisor: 7, note: "80 g scoop / pax / day → 1 tub ≈ 30 pax" },
    { cat: "ice-cream", role: "guest", name: /mochi/i,
      perPaxOnce: 2, unit: "box", note: "Each box ≈ 12 pcs, plan 2 boxes per dessert" },

    // ====================== FRUIT PURÉES (Ponthier 1 kg) ===================
    { cat: "ice-cream", role: "all", section: /fruit purées/i,
      perPaxOnce: 80, unit: "kg", minQty: 1, note: "1 kg purée serves ~12 desserts / cocktails" },

    // ====================== MODERNIST / SOSA ================================
    { cat: "modernist", role: "all", section: /hydrocolloids|sphérification|emulsifiers|ice-cream stabilisers/i,
      perPaxOnce: 1, unit: "g", minQty: 500, note: "1 × 500 g pack lasts a season" },
    { cat: "modernist", role: "all", section: /sugars|freeze-dried/i,
      perPaxOnce: 1, unit: "kg", minQty: 1, note: "1 kg / 500 g pack lasts a charter+" },
    { cat: "modernist", role: "guest", name: /gold leaf|silver leaf/i,
      perPaxOnce: 0.5, unit: "book", minQty: 1, note: "1 booklet (25 sheets) per ~50 plates" },
    { cat: "modernist", role: "all", name: /transglutaminase|activa/i,
      perPaxOnce: 1, unit: "kg", minQty: 1, note: "1 kg lasts months" },

    // ====================== GALLEY EQUIPMENT / DISPOSABLES ==================
    { cat: "equipment", role: "crew", section: /wraps|bags|paper/i,
      perPaxPerDay: 0.5, unit: "roll", minQty: 1, divisor: 30, note: "~1 roll per 60 pax-days" },
    { cat: "equipment", role: "crew", section: /cleaning chemicals|dishwasher|hand & body|cloths/i,
      perPaxOnce: 0.1, unit: "bottle", minQty: 1, note: "1 bottle / box per charter" },
    { cat: "equipment", role: "crew", name: /dishwasher tablet|powerball|quantum/i,
      perPaxPerDay: 3, unit: "box", minQty: 1, divisor: 110, note: "~3 tabs / pax / day → 1 box (110 tabs)" },
    { cat: "equipment", role: "crew", name: /latex glove|nitrile glove|vinyl glove/i,
      perPaxPerDay: 4, unit: "box", minQty: 1, divisor: 100, note: "~4 pairs / pax / day → 1 box / week" },
    { cat: "equipment", role: "all", name: /ziploc/i,
      perPaxOnce: 0.1, unit: "box", minQty: 1, note: "1 box per charter typical" },
    { cat: "equipment", role: "all", name: /vacuum-pack bag/i,
      perPaxOnce: 0.05, unit: "box", minQty: 1, note: "1 box (~1000 bags) lasts a season" },
    { cat: "equipment", role: "all", name: /smoke chip|charcoal|binchotan/i,
      perPaxOnce: 0.1, unit: "bag", minQty: 1, note: "1 bag per charter typical" },
    { cat: "equipment", role: "all", name: /gas cartridge|co2 cartridge|butane refill/i,
      perPaxOnce: 0.5, unit: "box", minQty: 1, note: "1 box / pack per charter" },

    // ====================== SPECIALTY MEAT ==================================
    { cat: "specialty-meat", role: "guest", name: /japanese a5 wagyu.*tenderloin|japanese a5 wagyu.*chateaubriand/i,
      perPaxOnce: 0.1, unit: "kg", minQty: 0.5, note: "100 g A5 / guest — single hero service" },
    { cat: "specialty-meat", role: "guest", name: /japanese a5 wagyu|japanese a4 wagyu|kobe beef/i,
      perPaxOnce: 0.12, unit: "kg", minQty: 0.5, note: "120 g A5 / guest" },
    { cat: "specialty-meat", role: "guest", name: /aussie wagyu 9\+|olive-fed wagyu/i,
      perPaxOnce: 0.18, unit: "kg", minQty: 0.5, note: "180 g 9+ Wagyu / guest" },
    { cat: "specialty-meat", role: "guest", name: /aussie wagyu 7\+|american wagyu/i,
      perPaxOnce: 0.2, unit: "kg", minQty: 0.5, note: "200 g 7+ Wagyu / guest" },
    { cat: "specialty-meat", role: "guest", section: /iberico/i,
      perPaxOnce: 0.18, unit: "kg", minQty: 0.5, note: "180 g Iberico / guest portion" },
    { cat: "specialty-meat", role: "guest", name: /jamón bellota.*pre-sliced/i,
      perPaxOnce: 0.4, unit: "tray", minQty: 1, note: "1 × 80 g tray per ~2.5 guests" },
    { cat: "specialty-meat", role: "guest", name: /whole leg jamón|paleta bellota/i,
      perPaxOnce: 0.05, unit: "pcs", minQty: 1, note: "1 leg per ~20 guests over week" },
    { cat: "specialty-meat", role: "guest", section: /dry-aged premium beef/i,
      perPaxOnce: 0.3, unit: "kg", minQty: 1, note: "300 g dry-aged per guest (bone deduction)" },
    { cat: "specialty-meat", role: "guest", name: /tomahawk|côte de bœuf/i,
      perPaxOnce: 0.5, unit: "kg", minQty: 1.2, note: "500 g shared bone-in per guest" },
    { cat: "specialty-meat", role: "guest", section: /heritage lamb/i,
      perPaxOnce: 0.25, unit: "pcs", minQty: 1, note: "1 piece per ~4 guests" },
    { cat: "specialty-meat", role: "guest", name: /wood pigeon|grouse|partridge|mallard duck/i,
      perPaxOnce: 1, unit: "pcs", minQty: 1, note: "1 whole bird per guest" },
    { cat: "specialty-meat", role: "guest", name: /^venison.*loin|^venison.*saddle/i,
      perPaxOnce: 0.18, unit: "kg", minQty: 1, note: "180 g venison loin / guest" },
    { cat: "specialty-meat", role: "all", name: /wild boar.*shoulder/i,
      perPaxOnce: 0.15, unit: "kg", minQty: 2, note: "150 g / pax (braised ragu)" },
    { cat: "specialty-meat", role: "guest", name: /^hare.*saddle/i,
      perPaxOnce: 0.5, unit: "pcs", minQty: 1, note: "1 saddle per 2 guests" },
    { cat: "specialty-meat", role: "all", name: /bresse chicken|label rouge chicken|capon|duck challans|duck rouen|guinea fowl/i,
      perPaxOnce: 0.25, unit: "pcs", minQty: 1, note: "1 bird per ~4 pax" },
    { cat: "specialty-meat", role: "guest", name: /poussin/i,
      perPaxOnce: 1, unit: "pcs", minQty: 1, note: "1 baby chicken per guest" },
    { cat: "specialty-meat", role: "guest", name: /duck breast magret|magret.*moulard/i,
      perPaxOnce: 0.5, unit: "pcs", minQty: 1, note: "1 magret per 2 guests" },
    { cat: "specialty-meat", role: "guest", name: /foie gras.*cru|raw goose|raw duck/i,
      perPaxOnce: 0.1, unit: "pcs", minQty: 1, note: "1 lobe (500 g) per ~10 guests" },
    { cat: "specialty-meat", role: "guest", name: /foie gras.*torchon|foie gras mi-cuit/i,
      perPaxOnce: 50, unit: "g", minQty: 200, note: "50 g torchon / guest service" },
    { cat: "specialty-meat", role: "guest", name: /foie gras.*iqf|foie gras.*escalope/i,
      perPaxOnce: 1, unit: "pack", minQty: 1, note: "1 pack (1 kg, ~16 escalopes) per service" },
    { cat: "specialty-meat", role: "guest", name: /truffle tuber magnatum|white alba/i,
      perPaxOnce: 5, unit: "g", minQty: 10, note: "5 g white Alba shaved / guest" },
    { cat: "specialty-meat", role: "guest", name: /truffle tuber melanosporum|black périgord|black perigord/i,
      perPaxOnce: 8, unit: "g", minQty: 20, note: "8 g black Périgord / guest" },
    { cat: "specialty-meat", role: "guest", name: /truffle tuber aestivum|summer truffle|tuber uncinatum|burgundy truffle/i,
      perPaxOnce: 10, unit: "g", minQty: 30, note: "10 g summer / Burgundy truffle / guest" },

    // ====================== PRODUCE — POPULARITY-RANKED PORTIONS ============
    // Yacht-chef demand x US consumer popularity. Differentiated by tier so
    // 3 kg tomato (Tier S) DOES order, but 3 kg endive (Tier C) DOES NOT.
    // Tier S (daily staples)   -> 30-80 g/pax/day   ~2-5 kg/week/10 pax
    // Tier A (crowd-pleasers)  -> 20-40 g/pax/day   ~1.5-3 kg/week/10 pax
    // Tier B (regular weekly)  -> 10-25 g/pax/day   ~700g-1.5 kg/week
    // Tier C (niche)           -> 5-15 g/pax/day    ~350-700 g/week
    // Tier D (garnish-only)    -> 1-2 packs/charter (minQty floor)
    // First-match wins - specific name rules first, section fallbacks last.

    // --- TIER S - Daily yacht staples ---
    { cat: "produce", role: "all", name: /^tomato/i,
      perPaxPerDay: 0.05, unit: "kg", minQty: 1, mainstream: true, note: "S - 50 g tomato / pax / day (salad + sauce + side)" },
    { cat: "produce", role: "all", name: /^cherry tomato|datterini|piccadilly/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, mainstream: true, note: "S - 25 g cherry tomato / pax / day" },
    { cat: "produce", role: "all", name: /^onion(?!s?\b.*spring|s?\b.*green|s?\b.*pearl)/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 1, mainstream: true, note: "S - 40 g onion / pax / day" },
    { cat: "produce", role: "all", name: /^shallot/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.5, mainstream: true, note: "S - 15 g shallot / pax / day" },
    { cat: "produce", role: "all", name: /^garlic(?!\s+chive)/i,
      perPaxPerDay: 0.008, unit: "kg", minQty: 0.5, mainstream: true, note: "S - 8 g garlic / pax / day" },
    { cat: "produce", role: "all", name: /^potato/i,
      perPaxPerDay: 0.08, unit: "kg", minQty: 2, mainstream: true, note: "S - 80 g potato / pax / day (sides + mash + roast)" },
    { cat: "produce", role: "all", name: /^carrot/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 1, mainstream: true, note: "S - 25 g carrot / pax / day" },
    { cat: "produce", role: "all", name: /^cucumber/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 1, mainstream: true, note: "S - 25 g cucumber / pax / day" },
    { cat: "produce", role: "all", name: /^bell pepper|^pepper.*(red|yellow|green|orange)|^capsicum/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 1, mainstream: true, note: "S - 25 g bell pepper / pax / day" },
    { cat: "produce", role: "all", name: /^lemon(?!grass)/i,
      perPaxPerDay: 0.3, unit: "pcs", minQty: 6, mainstream: true, note: "S - 1 lemon / 3 pax / day (cook + drink + garnish)" },
    { cat: "produce", role: "all", name: /^lime(?!stone|.*finger|.*makrut|.*kaffir|.*leaves)/i,
      perPaxPerDay: 0.2, unit: "pcs", minQty: 4, mainstream: true, note: "S - 1 lime / 5 pax / day" },
    { cat: "produce", role: "all", name: /^banana(?!\s+blossom|\s+leaf|\s+leaves|\s+flower)/i,
      perPaxPerDay: 0.4, unit: "pcs", minQty: 6, mainstream: true, note: "S - breakfast + smoothies + dessert" },
    { cat: "produce", role: "all", name: /^apple/i,
      perPaxPerDay: 0.35, unit: "pcs", minQty: 6, mainstream: true, note: "S - snack + dessert + sauce" },
    { cat: "produce", role: "all", name: /^strawberr/i,
      perPaxPerDay: 0.05, unit: "kg", minQty: 0.5, mainstream: true, note: "S - 50 g strawberry / pax / day" },
    { cat: "produce", role: "all", name: /^blueberr/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 0.5, mainstream: true, note: "S - 40 g blueberry / pax / day - #1 berry" },
    { cat: "produce", role: "all", name: /^avocado(?!\s+leaf|\s+leaves)/i,
      perPaxPerDay: 0.4, unit: "pcs", minQty: 6, mainstream: true, note: "S - breakfast + guac + plating" },
    { cat: "produce", role: "all", name: /butter lettuce|^bibb|^romaine|^iceberg|^little gem|^baby gem/i,
      perPaxPerDay: 0.25, unit: "heads", minQty: 4, mainstream: true, note: "S - 1 head / 4 pax / day" },

    // --- TIER A - Crowd-pleasers (2-3 services / week) ---
    { cat: "produce", role: "all", name: /^broccoli\b(?!ni)/i,
      perPaxPerDay: 0.03, unit: "kg", minQty: 1, mainstream: true, note: "A - 30 g broccoli / pax / day - top-3 US veg" },
    { cat: "produce", role: "all", name: /broccolini|tenderstem|sprouting broccoli/i,
      perPaxPerDay: 0.1, unit: "bunch", minQty: 2, mainstream: true, note: "A - 1 bunch / 10 pax / day" },
    { cat: "produce", role: "all", name: /baby spinach|^spinach/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, mainstream: true, note: "A - 25 g spinach / pax / day" },
    { cat: "produce", role: "all", name: /^white mushroom|^cremini|^button mushroom|^chestnut mushroom|portobello/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, mainstream: true, note: "A - 25 g common mushroom / pax / day" },
    { cat: "produce", role: "all", name: /^zucchini|courgette/i,
      perPaxPerDay: 0.03, unit: "kg", minQty: 1, mainstream: true, note: "A - 30 g zucchini / pax / day" },
    { cat: "produce", role: "all", name: /^asparagus.*green|^green asparagus|^asparagus$/i,
      perPaxPerDay: 0.15, unit: "bunch", minQty: 2, mainstream: true, note: "A - 1 bunch (~500g) / ~7 pax / day" },
    { cat: "produce", role: "all", name: /^raspberr/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, mainstream: true, note: "A - 25 g raspberry / pax / day" },
    { cat: "produce", role: "all", name: /^blackberr/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, mainstream: true, note: "A - 20 g blackberry / pax / day" },
    { cat: "produce", role: "all", name: /^mango(?!steen)/i,
      perPaxPerDay: 0.2, unit: "pcs", minQty: 3, mainstream: true, note: "A - 1 mango / 5 pax / day" },
    { cat: "produce", role: "all", name: /^pineapple/i,
      perPaxOnce: 0.15, unit: "pcs", minQty: 2, mainstream: true, note: "A - ~1.5 pineapple / 10 pax / week" },
    { cat: "produce", role: "all", name: /^watermelon(?!\s+radish)/i,
      perPaxOnce: 0.1, unit: "pcs", minQty: 1, mainstream: true, note: "A - 1 watermelon / 10 pax / week" },
    { cat: "produce", role: "all", name: /^cantaloupe|^honeydew|^charentais|^canary melon|^melon$/i,
      perPaxOnce: 0.25, unit: "pcs", minQty: 2, mainstream: true, note: "A - ~1 melon / 4 pax / week" },
    { cat: "produce", role: "all", name: /^orange(?! blossom)/i,
      perPaxPerDay: 0.2, unit: "pcs", minQty: 4, mainstream: true, note: "A - juice + dessert + carpaccio" },
    { cat: "produce", role: "all", name: /^grape(?!fruit)/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 0.5, mainstream: true, note: "A - 40 g grapes / pax / day - cheese board + snack" },
    { cat: "produce", role: "all", name: /^pear(?:s|\b)/i,
      perPaxPerDay: 0.2, unit: "pcs", minQty: 4, mainstream: true, note: "A - 1 pear / 5 pax / day" },
    { cat: "produce", role: "all", name: /^celery\b/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, mainstream: true, note: "A - 20 g celery / pax / day - mirepoix + crudite" },
    { cat: "produce", role: "all", name: /baby corn|^sweet corn|^corn on the cob/i,
      perPaxPerDay: 0.3, unit: "pcs", minQty: 4, mainstream: true, note: "A - 1 cob / ~3 pax / day" },
    { cat: "produce", role: "all", name: /^green bean|haricot vert|french bean/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 0.5, mainstream: true, note: "A - 40 g green beans / pax / day" },
    { cat: "produce", role: "all", name: /^pea(?!\s+shoot|\s+tendril)\b|sugar snap|snow pea/i,
      perPaxPerDay: 0.03, unit: "kg", minQty: 0.5, mainstream: true, note: "A - 30 g peas / pax / day" },

    // --- TIER B - Regular weekly use ---
    { cat: "produce", role: "all", name: /cauliflower.*white|^cauliflower\b(?!.*purple|.*romanesco)/i,
      perPaxPerDay: 0.12, unit: "heads", minQty: 1, mainstream: true, note: "B - 1 head / ~8 pax / day" },
    { cat: "produce", role: "all", name: /^kale\b/i,
      perPaxPerDay: 0.1, unit: "bunch", minQty: 1, mainstream: true, note: "B - 1 bunch / 10 pax / day" },
    { cat: "produce", role: "all", name: /^shiitake/i,
      perPaxPerDay: 0.012, unit: "kg", minQty: 0.3, note: "B - 12 g shiitake / pax / day" },
    { cat: "produce", role: "all", name: /^arugula|wild rocket/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.3, note: "B - 15 g arugula / pax / day - plate-finisher" },
    { cat: "produce", role: "all", name: /^basil\b|fresh basil/i,
      perPaxPerDay: 0.08, unit: "bunch", minQty: 1, mainstream: true, note: "B - 1 bunch / 12 pax / day" },
    { cat: "produce", role: "all", name: /^cilantro|^coriander.*fresh|fresh coriander/i,
      perPaxPerDay: 0.1, unit: "bunch", minQty: 1, note: "B - 1 bunch / 10 pax / day" },
    { cat: "produce", role: "all", name: /^parsley(?:\b|\s)/i,
      perPaxPerDay: 0.08, unit: "bunch", minQty: 1, note: "B - 1 bunch / 12 pax / day" },
    { cat: "produce", role: "all", name: /^mint(?:\b|\s)/i,
      perPaxPerDay: 0.06, unit: "bunch", minQty: 1, mainstream: true, note: "B - 1 bunch / ~16 pax / day" },
    { cat: "produce", role: "all", name: /^rosemary|^thyme|^oregano|^sage|^marjoram/i,
      perPaxPerDay: 0.04, unit: "bunch", minQty: 1, note: "B - 1 bunch / ~25 pax / day - hardy herb" },
    { cat: "produce", role: "all", name: /^ginger\b/i,
      perPaxPerDay: 0.005, unit: "kg", minQty: 0.2, mainstream: true, note: "B - 5 g ginger / pax / day" },
    { cat: "produce", role: "all", name: /^peach(?!es?.*frozen)/i,
      perPaxPerDay: 0.2, unit: "pcs", minQty: 4, mainstream: true, note: "B - 1 peach / 5 pax / day - seasonal" },
    { cat: "produce", role: "all", name: /^plum(?:s|\b)|^apricot(?:s|\b)|^nectarine/i,
      perPaxPerDay: 0.15, unit: "pcs", minQty: 4, note: "B - 1 stone fruit / 7 pax / day" },
    { cat: "produce", role: "all", name: /^kiwi/i,
      perPaxPerDay: 0.2, unit: "pcs", minQty: 4, mainstream: true, note: "B - 1 kiwi / 5 pax / day" },
    { cat: "produce", role: "all", name: /^pomegranate/i,
      perPaxOnce: 0.2, unit: "pcs", minQty: 2, note: "B - 1 pomegranate / 5 pax / week" },
    { cat: "produce", role: "all", name: /^leek/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.5, mainstream: true, note: "B - 15 g leek / pax / day" },
    { cat: "produce", role: "all", name: /^fennel/i,
      perPaxPerDay: 0.012, unit: "kg", minQty: 0.5, mainstream: true, note: "B - 12 g fennel / pax / day" },
    { cat: "produce", role: "all", name: /sweet potato|^yam(?:\b|s)/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, mainstream: true, note: "B - 25 g sweet potato / pax / day" },
    { cat: "produce", role: "all", name: /brussels sprout/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, mainstream: true, note: "B - 20 g Brussels sprouts / pax / day" },
    { cat: "produce", role: "all", name: /^aubergine|^eggplant/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, mainstream: true, note: "B - 20 g eggplant / pax / day" },
    { cat: "produce", role: "all", name: /^artichoke(?!.*jerusalem)/i,
      perPaxPerDay: 0.15, unit: "pcs", minQty: 4, note: "B - 1 artichoke / 7 pax / day" },
    { cat: "produce", role: "all", name: /^pak choy|^bok choy|^choy sum|^yu choy/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, note: "B - 25 g Asian green / pax / day" },
    { cat: "produce", role: "all", name: /^grapefruit/i,
      perPaxPerDay: 0.15, unit: "pcs", minQty: 3, note: "B - 1 grapefruit / 7 pax / day - breakfast" },
    { cat: "produce", role: "all", name: /^mandarin|^clementine|^tangerine/i,
      perPaxPerDay: 0.3, unit: "pcs", minQty: 6, mainstream: true, note: "B - 1 mandarin / 3 pax / day - snack" },
    { cat: "produce", role: "all", name: /^cherry\s+(?!tomato)|^cherries\b/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 0.5, note: "B - 40 g cherries / pax / day - seasonal" },
    { cat: "produce", role: "all", name: /^cabbage\s+savoy|^white cabbage|^green cabbage|^cabbage$/i,
      perPaxPerDay: 0.1, unit: "heads", minQty: 1, mainstream: true, note: "B - 1 head / 10 pax / day" },
    { cat: "produce", role: "all", name: /napa cabbage|chinese cabbage/i,
      perPaxPerDay: 0.12, unit: "heads", minQty: 1, note: "B - 1 head / 8 pax / day" },

    // --- TIER C - Niche / less crowd-pleasing ---
    { cat: "produce", role: "all", name: /^endive\b|^chicon\b|^belgian endive/i,
      perPaxPerDay: 0.06, unit: "heads", minQty: 2, note: "C - 1 head / 16 pax / day - bitter, niche" },
    { cat: "produce", role: "all", name: /radicchio|treviso|castelfranco|chioggia/i,
      perPaxPerDay: 0.04, unit: "heads", minQty: 1, note: "C - 1 head / 25 pax / day - bitter, niche" },
    { cat: "produce", role: "all", name: /^kohlrabi/i,
      perPaxPerDay: 0.04, unit: "pcs", minQty: 2, note: "C - 1 piece / 25 pax / day" },
    { cat: "produce", role: "all", name: /^beetroot|^beet\b(?!s? mix)|^beets\b/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.3, note: "C - 15 g beet / pax / day" },
    { cat: "produce", role: "all", name: /^turnip/i,
      perPaxPerDay: 0.008, unit: "kg", minQty: 0.3, note: "C - 8 g turnip / pax / day - braise only" },
    { cat: "produce", role: "all", name: /^parsnip/i,
      perPaxPerDay: 0.012, unit: "kg", minQty: 0.3, note: "C - 12 g parsnip / pax / day - winter" },
    { cat: "produce", role: "all", name: /celeriac|celery root/i,
      perPaxPerDay: 0.012, unit: "kg", minQty: 0.5, note: "C - 12 g celeriac / pax / day - puree + remoulade" },
    { cat: "produce", role: "all", name: /^radish\b/i,
      perPaxPerDay: 0.05, unit: "bunch", minQty: 1, note: "C - 1 bunch / 20 pax / day - crudite" },
    { cat: "produce", role: "all", name: /jerusalem artichoke|sunchoke|topinambour/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.5, note: "C - 15 g sunchoke / pax / day - puree + roast" },
    { cat: "produce", role: "all", name: /^chard|swiss chard|silverbeet/i,
      perPaxPerDay: 0.04, unit: "bunch", minQty: 1, note: "C - 1 bunch / 25 pax / day" },
    { cat: "produce", role: "all", name: /watercress/i,
      perPaxPerDay: 0.05, unit: "bunch", minQty: 1, note: "C - 1 bunch / 20 pax / day - plate garnish" },
    { cat: "produce", role: "all", name: /^dill\b|^chervil|^tarragon|^chives\b|^sorrel/i,
      perPaxPerDay: 0.05, unit: "bunch", minQty: 1, note: "C - 1 bunch / 20 pax / day - garnish herb" },
    { cat: "produce", role: "all", name: /^fig(?:s|\b)|fresh fig/i,
      perPaxPerDay: 0.2, unit: "pcs", minQty: 4, note: "C - 1 fig / 5 pax / day - seasonal" },
    { cat: "produce", role: "all", name: /persimmon|sharon fruit|kaki/i,
      perPaxPerDay: 0.15, unit: "pcs", minQty: 3, note: "C - 1 persimmon / 7 pax / day - seasonal" },
    { cat: "produce", role: "all", name: /passion fruit/i,
      perPaxPerDay: 0.15, unit: "pcs", minQty: 4, note: "C - 1 passion / 7 pax / day" },
    { cat: "produce", role: "all", name: /^quince/i,
      perPaxOnce: 0.15, unit: "pcs", minQty: 3, note: "C - ~1.5 quince / 10 pax / week - poaching" },
    { cat: "produce", role: "all", name: /^okra|^bhindi|^lady('s)? finger/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, note: "C - 20 g okra / pax / day - regional" },
    { cat: "produce", role: "all", name: /^purple cauliflower|^romanesco/i,
      perPaxPerDay: 0.08, unit: "heads", minQty: 1, note: "C - 1 head / 12 pax / day - less-common variety" },
    { cat: "produce", role: "all", name: /^rhubarb/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, note: "C - 25 g rhubarb / pax / day - compote / spring" },

    // --- TIER D - Garnish-only / hyper-niche ---
    { cat: "produce", role: "all", name: /^mache|lamb('s)? lettuce/i,
      perPaxOnce: 0.1, unit: "bag", minQty: 1, note: "D - 1-2 bags / 10 pax / week - plate garnish" },
    { cat: "produce", role: "all", name: /^frisee|curly endive/i,
      perPaxOnce: 0.1, unit: "heads", minQty: 1, note: "D - 1 head / 10 pax / week - garnish" },
    { cat: "produce", role: "all", name: /^micro\b|baby leaves\b|baby cress|baby kale|baby radish leaves|baby chard|baby herbs|baby sorrel/i,
      perPaxOnce: 0.15, unit: "pack", minQty: 2, note: "D - 2 packs / charter - plating only" },
    { cat: "produce", role: "all", name: /edible flower|nasturtium|pansy|viola|borage|marigold|rose petal|courgette flower|squash flower/i,
      perPaxOnce: 0.1, unit: "pack", minQty: 1, note: "D - 1-2 packs / charter - finishing only" },
    { cat: "produce", role: "all", name: /pea shoot|pea tendril|sprouted|^sprouts/i,
      perPaxOnce: 0.15, unit: "pack", minQty: 2, note: "D - 1-2 packs / charter - garnish" },
    { cat: "produce", role: "all", name: /dragon ?fruit|pitaya/i,
      perPaxOnce: 0.1, unit: "pcs", minQty: 2, note: "D - exotic - dessert plate" },
    { cat: "produce", role: "all", name: /finger lime|caviar lime/i,
      perPaxOnce: 0.05, unit: "pcs", minQty: 2, note: "D - 1 / 20 pax - luxury garnish" },
    { cat: "produce", role: "all", name: /^yuzu\b(?!.*juice|.*puree)|^sudachi|^kabosu|^kalamansi(?!.*juice)|^calamansi/i,
      perPaxOnce: 0.1, unit: "pcs", minQty: 2, note: "D - rare citrus / garnish" },
    { cat: "produce", role: "all", name: /buddha('s)? hand|fingered citron/i,
      perPaxOnce: 0.05, unit: "pcs", minQty: 1, note: "D - 1 per charter - zest only" },
    { cat: "produce", role: "all", name: /rambutan|mangosteen|cherimoya|durian|jackfruit/i,
      perPaxOnce: 0.2, unit: "pcs", minQty: 3, note: "D - exotic tropical / dessert" },
    { cat: "produce", role: "all", name: /^lychee\b(?!.*can|.*jar|.*juice|.*puree)/i,
      perPaxOnce: 0.05, unit: "kg", minQty: 0.5, note: "D - fresh lychee - seasonal Asian dessert" },
    { cat: "produce", role: "all", name: /^longan|^rambai/i,
      perPaxOnce: 0.05, unit: "kg", minQty: 0.5, note: "D - exotic Asian fruit" },
    { cat: "produce", role: "all", name: /salsify|scorzonera|cardoon/i,
      perPaxOnce: 0.05, unit: "kg", minQty: 0.5, note: "D - very niche root - winter" },
    { cat: "produce", role: "all", name: /samphire|sea aster|sea purslane|oyster leaf|ice plant|sea fennel/i,
      perPaxOnce: 0.05, unit: "pack", minQty: 1, note: "D - sea vegetable garnish - 1 pack / charter" },

    // --- FALLBACK - section-scoped catch-alls (last) ---
    // Anything in produce that no name rule matched gets a conservative Tier-B-ish default.
    // Section fallbacks for produce sections where item names don't start with the common name
    { cat: "produce", role: "all", section: /^tomatoes/i,
      perPaxPerDay: 0.04, unit: "kg", minQty: 1, mainstream: true, note: "S - tomato section · 40 g / pax / day" },
    { cat: "produce", role: "all", section: /^peppers|peppers — sweet/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, mainstream: true, note: "S - pepper section · 25 g / pax / day" },
    { cat: "produce", role: "all", section: /squash|gourd|marrow/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, mainstream: true, note: "B - squash · 25 g / pax / day" },
    { cat: "produce", role: "all", section: /aubergine|eggplant|fennel|mediterranean/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, mainstream: true, note: "B - mediterranean veg · 20 g / pax / day" },
    { cat: "produce", role: "all", section: /melons|grapes/i,
      perPaxOnce: 0.2, unit: "pcs", minQty: 2, mainstream: true, note: "A - melon/grape mix · 1 / 5 pax / week" },
    { cat: "produce", role: "all", section: /leafy greens/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.3, mainstream: true, note: "fallback - leafy 20 g / pax / day" },
    { cat: "produce", role: "all", section: /brassicas/i,
      perPaxPerDay: 0.02, unit: "kg", minQty: 0.5, mainstream: true, note: "fallback - brassicas 20 g / pax / day" },
    { cat: "produce", role: "all", section: /roots|tubers/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.5, mainstream: true, note: "fallback - roots 15 g / pax / day" },
    { cat: "produce", role: "all", section: /alliums/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.5, mainstream: true, note: "fallback - alliums 15 g / pax / day" },
    { cat: "produce", role: "all", section: /fruiting vegetable|nightshades/i,
      perPaxPerDay: 0.025, unit: "kg", minQty: 0.5, mainstream: true, note: "fallback - fruiting veg 25 g / pax / day" },
    { cat: "produce", role: "all", section: /^herbs|fresh herbs|aromatics/i,
      perPaxPerDay: 0.05, unit: "bunch", minQty: 1, mainstream: true, note: "fallback - 1 bunch / 20 pax / day" },
    { cat: "produce", role: "all", section: /berries/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.5, mainstream: true, note: "fallback - berries 15 g / pax / day" },
    { cat: "produce", role: "all", section: /citrus/i,
      perPaxPerDay: 0.15, unit: "pcs", minQty: 4, mainstream: true, note: "fallback - citrus 1 / 7 pax / day" },
    { cat: "produce", role: "all", section: /tropical|exotic/i,
      perPaxOnce: 0.15, unit: "pcs", minQty: 2, note: "fallback - tropical 1 / 7 pax / week" },
    { cat: "produce", role: "all", section: /stone fruit|orchard|pome/i,
      perPaxPerDay: 0.15, unit: "pcs", minQty: 4, mainstream: true, note: "fallback - stone fruit 1 / 7 pax / day" },
    { cat: "produce", role: "all", section: /mushroom/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.3, mainstream: true, note: "fallback - mushroom 15 g / pax / day" },
    { cat: "produce", role: "all", section: /asian|tropical veg|specialty/i,
      perPaxPerDay: 0.015, unit: "kg", minQty: 0.3, note: "fallback - specialty veg 15 g / pax / day" },
    { cat: "produce", role: "all", section: /sprouts|microgreens|edible flowers/i,
      perPaxOnce: 0.15, unit: "pack", minQty: 1, note: "fallback - 1-2 packs / charter" },
  ];
  ...
  function enrichItem(item, cat, section, ctx) {
    ...
    for (const rule of portions) {
      ...
      if (pax === 0) { continue; } // wrong role for current PAX — try next rule
      ...
      if (qty != null) {
        const wholeUnits = new Set(["tin","tins","pcs","pc","piece","jar","jars","bunch","bunches","case","box","boxes","pack","packs","birds","racks","legs","claws","side","sides","lobes","tray","trays","log","logs","sheets","wheel","stick","sticks","book","books","tube","tubes","bottle","bottles","cans","pods","heads","head","stalk","stalks","bag","bags","hank","hanks","roll","rolls","set","sets","sachet","sachets","pair","pairs","tub","tubs"]);
        ...
        // Always round UP (ceil) — chef would rather have a bit extra than run out.
        if (rule.unit === "kg") qty = Math.ceil(qty * 10) / 10;
        ...
        result.portion = { qty, unit: rule.unit, note: rule.note, role, math: mathLine, isMin: appliedMin, mainstream: rule.mainstream === true };
      }
      break;
    }
