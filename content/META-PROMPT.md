...
| 02 | Technique | One technique — provoked, instructed, proved | **The failure everyone has had** — the standard approach and the break it produces | Provoke→Instruct→Prove arc + threshold table + lineage note | The threshold number on a primary paper; an interactive explainer where the technique earns one (self-explanatory, reader-driven) | Recipe content (Recipe Blueprint's, 05); steps without mechanism |
| 03 | Heritage | A dish's documented life, told warm | **A dated scene** — a real recorded moment (year, place, hands), watched | Heritage Timeline (the only template where time passes) + Origin Card + Tradition Ledger | **Documented-vs-legend** — the origin story pros repeat, corrected against the record | Undated round-number history; ingredient-profile machinery |
| 04 | Study Decoded | One paper, decoded to one galley move | **The finding as a usable sentence** — actable tonight | Citation Card with grade chip; the 1–2 min discipline | "What Cooks Already Knew" + honest limitations (calibration, not just facts) | Multi-study essays; running past 2 min; any stacked box |
...
| 11 | Equipment Review | Gear judged against the job, counted | **The job it has to do** — the galley problem, precisely defined | `glance` spec grid + counted test + unambiguous verdict + when-not-to | The claim-vs-physics check + the trial data, conditions named | Brochure rewrites; softened verdicts; specs without price/footprint/draw |
