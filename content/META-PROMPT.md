...
|---|---|---|---|---|---|---|
| 01 | Shore Larder | The fast, warm field profile of one ingredient | **In scene at the encounter** — dock, crate, market, pan; 1–2 sentences, then The Cook's Reason | The Cook's Reason + vital-signs `glance` | **The Blind Spot** — what most pros have wrong about this everyday ingredient, opening The Science | The deep-dive dossier (id-card, register ladder, compound table); a second stat grid |
| 01b | Shore Larder Deep-Dive | The laboratory dossier: one ingredient, four registers | **The paradox** — the ingredient's central contradiction, stated in two lines | 8-cell Identity Card + Four Ways + register ladder + compound table | The Frontier register + shared-molecule bridges | Shipping without Four Ways; filler examples ("several only where notable") |
| 02 | The Method | One technique — provoked, instructed, proved | **The failure everyone has had** — the standard approach and the break it produces | Provoke→Instruct→Prove arc + threshold table + lineage note | The threshold number on a primary paper; an interactive explainer where the technique earns one (self-explanatory, reader-driven) | Recipe content (Blueprint's); steps without mechanism |
| 03 | Littoral Heritage | A dish's documented life, told warm | **A dated scene** — a real recorded moment (year, place, hands), watched | Heritage Timeline (the only template where time passes) + Origin Card + Tradition Ledger | **Documented-vs-legend** — the origin story pros repeat, corrected against the record | Undated round-number history; ingredient-profile machinery |
| 04 | The Evidence | One paper, decoded to one galley move | **The finding as a usable sentence** — actable tonight | Citation Card with grade chip; the 1–2 min discipline | "What Cooks Already Knew" + honest limitations (calibration, not just facts) | Multi-study essays; running past 2 min; any stacked box |
| 05 | The Blueprint | A recipe engineered: ratio, science, two services | **The sensory payoff** — headnote sentence one, what the dish does when it works | The `rcp` engine + two galley cards + "Did you know" | The ratio (memorise once, rescale anything) + one measured number with DOI | Re-ordering ings/steps post-publish; baker's % outside the ratio box; touching the locked card design |
| 06 | Signal Fire | The flare — act this week | **The number that moved, dated** — the lede is the signal | Signal Board (3 cells) + The Move, early | **Where It Really Comes From** — real provenance vs label, who clips the margin | Future-tense calls (Horizon's); an essay without the board |
| 07 | The Horizon | The forecast that argues with itself | **The bet, stated** — thesis, no hedging | Forecast Scorecard + "Falsifies if" + case-for/case-against | The falsifier + watch-list thresholds (teaches reading the indicators) | Unfalsifiable calls (don't ship); "certain"; act-now framing (Signal Fire's) |
| 08 | Trade Winds | The rulebook moving under your feet, by region | **The dockside myth** — what everyone repeats, and the shift making it dangerous | The comparison rack (case-grid / hub-grid / band-ladder) | The named statute + exact threshold pros half-know + The Trap | Hand-rolled grids; advice without the named rule; one-rule-fits-all |
| 09 | Tight Ship | A protocol that survives swell — printable | **The constraint in hard numbers** — m², covers, minutes; a design parameter | The Station Map (`galley-plan`) + print actions | Proof-it-scales counts ("held 14 of 16") + the daily non-negotiable, timed | An essay without the map; vibes instead of counts |
| 10 | Port Call | The port, decoded from the berth | **The port's single hard fact** — the constraint that shapes every decision | Quick Map text table + Three Berths + supplier folds + Cook This Port | The one number repeated at the pass (cold-chain figure, named agency) + price-vs-import | Graphic maps (Terroir's); tourism prose; unverified suppliers |
| 11 | The Locker | Gear judged against the job, counted | **The job it has to do** — the galley problem, precisely defined | `glance` spec grid + counted test + unambiguous verdict + when-not-to | The claim-vs-physics check + the trial data, conditions named | Brochure rewrites; softened verdicts; specs without price/footprint/draw |
