...
Card), **11** (Since last week). The Supply Alert & Forecast opens on the dated number and the Signal
Board, then carries the sum-up. Never copy another template's opener. `tools/lint.mjs` checks the
...
|---|---|---|---|---|---|---|
| 01 | Ingredient Profile | The fast, warm field profile of one ingredient — the only ingredient template | **The specimen case, first on the page** — the sketch on the glass, the Latin name, one hint line; the name and the origin folded inside; then the taste, closing on the hand | The specimen case + "In the mouth" + the four numbers in The Science + the pairing cards with the littoral wild card + the galley tiles + the varieties as cards | **The Blind Spot** — what most pros have wrong about this everyday ingredient, opening The Science | A scene line above the case; a promise box; a Cook's Reason box; a Stories section; the old deep-dive dossier (id-card, register ladder, compound table); a second stat grid; an unsourced folk etymology |
...
| 06 | Supply Alert & Forecast | One signal, now and ahead — the flare, then the call that argues with itself | **The number that moved, dated** — the lede is the signal | Signal Board (now: number / direction / window) + The Move, early + Forecast Scorecard (ahead: the call / horizon / confidence + "Falsifies if") + the Signal Ledger + the case against + the Watch List | **Where It Really Comes From** — real provenance vs label, who clips the margin; the falsifier + watch-list thresholds | An essay without the board; unfalsifiable calls (don't ship); "certain"; fake urgency; a call with no report-back date |
...
| 08 | Galley Operations | A protocol that survives swell — printable | **The constraint in hard numbers** — m², covers, minutes; a design parameter | The Station Map (`galley-plan`) + print actions | Proof-it-scales counts ("held 14 of 16") + the daily non-negotiable, timed | An essay without the map; vibes instead of counts |
